...
class WechatPaymentApi
{
	 ...
	 * @param int $timeOut
	 * @param WechatPaymentConfig $WxCfg
	 * @throws DEVDIY_Wx_Pay_Exception
	 * @return 成功时返回，其他抛异常
	 */
	public static function refund($inputObj, $timeOut = 60,$WxCfg)
	{
		...
		//检测必填参数
		if(!$inputObj->IsOut_trade_noSet() && !$inputObj->IsTransaction_idSet()) {
			throw new DEVDIY_Wx_Pay_Exception("退款申请接口中，out_trade_no、transaction_id至少填一个！");
		}else if(!$inputObj->IsOut_refund_noSet()){
			throw new DEVDIY_Wx_Pay_Exception("退款申请接口中，缺少必填参数out_refund_no！");
		}else if(!$inputObj->IsTotal_feeSet()){
			throw new DEVDIY_Wx_Pay_Exception("退款申请接口中，缺少必填参数total_fee！");
		}else if(!$inputObj->IsRefund_feeSet()){
			throw new DEVDIY_Wx_Pay_Exception("退款申请接口中，缺少必填参数refund_fee！");
		}else if(!$inputObj->IsOp_user_idSet()){
			throw new DEVDIY_Wx_Pay_Exception("退款申请接口中，缺少必填参数op_user_id！");
		}
		$inputObj->SetAppid($WxCfg->getAPPID());//公众账号ID
		$inputObj->SetMch_id($WxCfg->getMCHID());//商户号
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
